...
## Purpose 
This project is designed to provide a method of extracting syscalls dynamically directly from on-disk ntdll to cyber researchers to aid in userland unhooking.  Userland hooks have become prevalent in many security products these days, and bypassing these hooks is a great way for red teamers/pentesters to bypass these defenses.  Many great projects have recently covered this topic, including the Hell's Gate Project (https://github.com/am0nsec/HellsGate), Freshycalls (https://github.com/Crummie5/Freshycalls_PoC/, https://www.crummie5.club/freshycalls/#and-finally-our-library), ShellyCoat (https://github.com/slaeryan/AQUARMOURY/blob/master/Shellycoat/README.md), and the SysWhispers project (https://github.com/jthuraisamy/SysWhispers).  A great explanation of unhooking based on mapping a clean copy of ntdll over a hooked one can be found at (https://www.solomonsklash.io/pe-parsing-defeating-hooking.html).  This, as well as the Hell's Gate paper, does a great job at walking users through PE parsing and what it will accomplish - a lot of DoppelGate code is borrowed directly from Hell's Gate.  This project does not aim to re-hash all of those details, especially since PE information and other syscall extraction techniques are very prevalent.  
...
2.  Copy ntdll to an arbitrary dll in an arbitrary directory you will have write access to using CopyFileTransactedA on ntdll.  Use NtReadFile on your arbitrary dll vs ntdll.  Interestingly, this approach tends to be much louder, as many calls to NtReadFile, NtCreateFile, and NtWriteFile will be made in order to copy the data from ntdll to your arbitary dll.  However, assuming you don't commit the transaction, your arbitrary dll never actually exists on disk, which was an interesting discovery.  

3.  You could always call NtCreateFile on ntdll directly and then use NtReadFile on the resulting handle - this does not get around the NtReadFile hook problem. 

NOTE:  As pointed out by others in their various blog posts/projects, the problem with this approach is that if a defensive product hooks NtReadFile, and catches that you are reading ntdll.dll, they can block you.   This is true, and this approach DOES NOT solve this problem.  I was curious if Transactions could be used to slightly obfuscate the technique, but it appears not to be the case.  

With this in mind, I recommend that if you want to utilize this technique, you combine it with another of the aforementioned techniques first, preferably hard-coded syscalls or Hell's Gate.  Hell's Gate -> DoppelGate would likely give you the most robust solution with little possibility of error, since all syscalls would be fetched dynamically for you. Hard-coded syscalls -> DoppelGate might be slightly safer from an opsec perspective, but more prone to failure if your initial hard-coded syscalls are wrong for the operating system.  You need to obfuscate 2 (or 3 at most) functions to use DoppelGate with lower (never say never) rates of detection, and these Nt functions are: NtCreateTransaction, NtReadFile, and optionally NtCreateFile (if using the 3rd approach listed above).  In the code sample I have included ways to both only use DoppelGate as well as to combine SysWhispers with DoppelGate as an example (to use SysWhispers comment out the ntdll.lib line in Header.h, and include SysWhispers.h and SysWhispers.asm in the compilation of DoppelGate).  Only the first two approaches listed above exist in the code, but the third could easily be introduced.  

2nd NOTE:  DoppelGate does not rely on CreateFileMapping to map the bytes from ntdll into memory, but parses through the PE in memory directly.  This requires slightly more work, and was definitely a fun challenge.

If you are interested in the technical details, please proceed to the Technical Nitty Gritty section below.


## Detection/Prevention
If defense products are tuned to prevent NTFS Transactions or flag on them (similar to Process Doppelganging, especially on CreateFileTransacted and CopyFileTranscated), that would provide insight into/prevent this method (at least the first two approaches) being used.  As mentioned previously, hooks on NtReadFile will also prevent this technique, unless other obfuscation is used, as it is weird to parse ntdll directly from disk.
...
Apologies in advance for not the cleanest coding! :D
